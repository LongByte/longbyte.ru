    ...
    if ($arParams["ELEMENT_ID"] > 0) {
        ...
        if ($obElement = $rsElement->GetNextElement()) {
            ...
            $arResult['LINK_URL'] = str_replace(
                array("#ELEMENT_ID#", "#SECTION_ID#"), array($arResult["ID"], $arResult["SECTION"]["ID"]), $arParams["LINK_ELEMENTS_URL"]
            );
            ...
            if ($bCatalog) {
                // catalog hit stats
                $productTitle = !empty($arResult["IPROPERTY_VALUES"]["ELEMENT_PAGE_TITLE"]) ? $arResult["IPROPERTY_VALUES"]["ELEMENT_PAGE_TITLE"] : $arResult["NAME"];
...
if (isset($arResult["ID"])) {
    ...
    if ($USER->IsAuthorized()) {
        if (
            $APPLICATION->GetShowIncludeAreas() || $arParams["SET_TITLE"] || isset($arResult[$arParams["BROWSER_TITLE"]])
        ) {
            if (Loader::includeModule('iblock')) {
                $arReturnUrl = array(
    ...
    if ($arParams['SET_VIEWED_IN_COMPONENT'] == 'Y' && !empty($arResult['VIEWED_PRODUCT'])) {
        if (Loader::includeModule('catalog') && Loader::includeModule('sale')) {
            \Bitrix\Catalog\CatalogViewedProductTable::refresh(
                $arResult['VIEWED_PRODUCT']['OFFER_ID'], CSaleBasket::GetBasketUserID(), SITE_ID, $arResult['VIEWED_PRODUCT']['PRODUCT_ID']
            );
        }
    }

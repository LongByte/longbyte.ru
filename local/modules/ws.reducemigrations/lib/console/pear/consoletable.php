...
class ConsoleTable {
    ...
    private function buildTable() {
        ...
        for ($i = 0; $i < count($this->data); $i++) {
            ...
            if ($this->data[$i] !== CONSOLE_TABLE_HORIZONTAL_RULE) {
                $row_begin = $this->borderVisibility['left'] ? $vertical . str_repeat(' ', $this->padding) : '';
                $row_end = $this->borderVisibility['right'] ? str_repeat(' ', $this->padding) . $vertical : '';
    ...
    private function getSeparator() {
        ...
        $row_begin = $this->borderVisibility['left'] ? $intersection . str_repeat($horizontal, $this->padding) : '';
        $row_end = $this->borderVisibility['right'] ? str_repeat($horizontal, $this->padding) . $intersection : '';
        ...
        for ($j = 0; $j < count($this->headers); $j++) {
            for ($i = 0; $i < count($this->headers[$j]); $i++) {
                if ($this->strlen($this->headers[$j][$i]) <
                    ...
                    $this->headers[$j][$i] = $this->strpad($this->headers[$j][$i],
                        $this->getCellLength($i),
                        ' ',
                        $this->colAlign[$i]);
                }

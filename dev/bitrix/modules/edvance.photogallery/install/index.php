...
if (class_exists("edvance_photogallery")) return;

Class edvance_photogallery extends CModule
{
	var $MODULE_ID = "edvance.photogallery";
	var $MODULE_VERSION;
	...
	function InstallDB()
	{
		RegisterModule("edvance.photogallery");
	...
	function UnInstallDB()
	{
		...
		UnRegisterModule("edvance.photogallery");
		return true;
	}
	...
	function InstallFiles()
	{
		...
		//{
			CopyDirFiles($_SERVER["DOCUMENT_ROOT"]."/bitrix/modules/edvance.photogallery/install/components", $_SERVER["DOCUMENT_ROOT"]."/bitrix/components", true, true);
		//}
		return true;
	}
	...
	function DoInstall()
	{
		...
		if (IsModuleInstalled("iblock"))
		{
			...
			if ($step < 2)
				$APPLICATION->IncludeAdminFile(GetMessage("PHOTO_INSTALL"), $_SERVER["DOCUMENT_ROOT"]."/bitrix/modules/edvance.photogallery/install/step1.php");
			elseif($step == 2)
				$APPLICATION->IncludeAdminFile(GetMessage("PHOTO_INSTALL"), $_SERVER["DOCUMENT_ROOT"]."/bitrix/modules/edvance.photogallery/install/step2.php");
			elseif ($step == 3)
			{
				$this->InstallDB();
				$this->InstallEvents();
		...
		elseif (!IsModuleInstalled("edvance.photogallery"))
		{
			$this->InstallDB();
			$this->InstallEvents();
			$this->InstallFiles();

...
if (!empty($arResult["ERRORS"])):
    echo str_replace("'", "", implode("<br />", $arResult["ERRORS"]));
elseif (strlen($arResult["MESSAGE"]) > 0):
    echo $arResult["MESSAGE"];
endif
?>
<form name="iblock_add" action="<?= $APPLICATION->GetCurPage() ?>" method="post" enctype="multipart/form-data" class="iblock-element-add-form sell">
    <?= bitrix_sessid_post() ?>
    <? if ($arParams["MAX_FILE_SIZE"] > 0): ?><input type="hidden" name="MAX_FILE_SIZE" value="<?= $arParams["MAX_FILE_SIZE"] ?>" /><? endif ?>
    <table class="data-table">
        <thead>
            <tr>
                <td colspan="2">Напишите нам</td>
            </tr>
        </thead>
        <? if (is_array($arResult["PROPERTY_LIST"]) && !empty($arResult["PROPERTY_LIST"])): ?>
            <tbody>
                <? foreach ($arResult["PROPERTY_LIST"] as $propertyID): ?>
                    <tr>
                        <td>
                            <? if (!in_array($arResult["PROPERTY_LIST_FULL"][$propertyID]["PROPERTY_TYPE"], array("E", "G"))): ?>
                            ...
                            switch ($INPUT_TYPE):
                                case "USER_TYPE":
                                    for ($i = 0; $i < $inputNum; $i++) {
                                        ...
                                        if ($arResult["PROPERTY_LIST_FULL"][$propertyID]["CODE"] == "COMMENT") {
                                            ?>
                                            <textarea rows="4" name="PROPERTY[<?= $propertyID ?>][<?= $i ?>][VALUE][TEXT]" placeholder="<?= $arResult["PROPERTY_LIST_FULL"][$propertyID]["HINT"] ?>"><?= $value ?></textarea>
                                            <?
                                        } else {
                                ...
                                case "G":
                                    ?>
                                    <input type="hidden" name="PROPERTY[<?= $propertyID ?>][0]" value="<?= $value ?>" /><br />
                                    <?
                                    break;
                                case "T":
                                    for ($i = 0; $i < $inputNum; $i++) {
                                ...
                                case "N":
                                    for ($i = 0; $i < $inputNum; $i++) {
                                        ...
                                        if ($propertyID == "NAME") {
                                            $arResult["PROPERTY_LIST_FULL"][$propertyID]["HINT"] = "Иван Иванов";
                                        }
                                        ?>
                                        <input type="text" name="PROPERTY[<?= $propertyID ?>][<?= $i ?>]" size="25" value="<?= $value ?>" placeholder="<?= $arResult["PROPERTY_LIST_FULL"][$propertyID]["HINT"] ?>"/><br /><? if ($arResult["PROPERTY_LIST_FULL"][$propertyID]["USER_TYPE"] == "DateTime"): ?><?
                                            $APPLICATION->IncludeComponent(

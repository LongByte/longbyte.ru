...
CModule::IncludeModule('realweb.helper');
...
if (isset($_REQUEST['IS_CRON'])) {
    $_POST = $_REQUEST;
}

$arSiteMap = false;
if (isset($_REQUEST['IS_CRON']) || $_SERVER["REQUEST_METHOD"] == "POST" && $_REQUEST["Generate"] == "Y") {
    require_once($_SERVER["DOCUMENT_ROOT"] . "/bitrix/modules/main/include/prolog_admin_js.php");

    if (isset($_REQUEST['IS_CRON']) || check_bitrix_sessid()) {
    ...
    <script language="JavaScript">
        ...
        function DoNext(NS)
        {
            ...
            if (!stop)
            {
                ShowWaitWindow();
                BX.ajax.post(
                    'realweb_sitemap.php?' + queryString,
                    NS,
                    function (result) {
    ...
    <form method="POST" action="<? echo $APPLICATION->GetCurPage() ?>?lang=<? echo htmlspecialcharsbx(LANG) ?>" name="fs2">
        ...
        <? if (IsModuleInstalled("forum")): ?>
            <tr>
                ...
                <td><input type="checkbox" id="sm_forum_topics_only" name="sm_forum_topics_only" value="Y"<? if (COption::GetOptionString("search", "sm_forum_topics_only") == "Y") echo ' checked="checked"' ?>></td>
            </tr>
        <? endif ?>
        <? if (IsModuleInstalled("blog")): ?>
            <tr>
                <td><label for="sm_blog_no_comments"><? echo GetMessage("SEARCH_SITEMAP_BLOG_NO_COMMENTS") ?>:</label></td>
                <td><input type="checkbox" id="sm_blog_no_comments" name="sm_blog_no_comments" value="Y"<? if (COption::GetOptionString("search", "sm_blog_no_comments") == "Y") echo ' checked="checked"' ?>></td>
            </tr>
        <? endif ?>
        <tr>
            <td><label for="sm_use_https"><? echo GetMessage("SEARCH_SITEMAP_USE_HTTPS") ?>:</label></td>
            <td><input type="checkbox" id="sm_use_https" name="sm_use_https" value="Y"<? if (COption::GetOptionString("search", "sm_use_https") == "Y") echo ' checked="checked"' ?>></td>
        </tr>
        <?
        $tabControl->Buttons();
        ?>

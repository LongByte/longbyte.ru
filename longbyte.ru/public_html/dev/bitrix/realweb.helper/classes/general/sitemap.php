...
class CAllSiteMapRealweb extends CDBResult {
    ...
    function Create($site_id, $max_execution_time, $NS, $arOptions = array()) {
        ...
        if ($arSite = $rsSite->Fetch()) {
            ...
            while (!$bFileIsFull && $ar = $this->Fetch()) {
                ...
                if ($ar["MODULE_ID"] == "iblock" && $ar["ITEM_ID"]) {
                ...
                foreach ($this->arRules as &$arRule) {
                    $match = false;
                    $ruleType = substr($arRule['URL'], 0, 1);
                    switch ($ruleType) {
                        case '=':
                            if ($ar["URL"] == substr($arRule['URL'], 1)) {
                                $match = true;
                            }
                            break;
                        case '~':
                            if (preg_match(substr($arRule['URL'], 1), $ar["URL"])) {
                                $match = true;
                            }
                            break;
                        default :
                            if (strpos($ar["URL"], $arRule['URL']) === 0) {
                                $match = true;
                            }
                            break;
                    }
                    if ($match) {
                        $priority = $arRule['PRIORITY'];
                        $changefreq = $arRule['CHANGEFREQ'];
                        $banrule = $arRule['BANRULE'];
                        if ($banrule)
                            continue 2;
                        break;
                    }
    ...
    function Fetch() {
        ...
        if ($r) {
            ...
            $r["URL"] = str_replace(
                array("#LANG#", "#SITE_DIR#", "#SERVER_NAME#"), array($r["DIR"], $r["DIR"], $r["SERVER_NAME"]), $r["URL"]
            );

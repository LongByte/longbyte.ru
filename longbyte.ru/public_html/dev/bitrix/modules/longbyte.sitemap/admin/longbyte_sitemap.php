...
while ($sitemap = $data->NavNext()) {
    ...
    $row = &$adminList->AddRow($sitemap["ID"], $sitemap, "longbyte_sitemap_edit.php?ID=" . $sitemap["ID"] . "&lang=" . LANGUAGE_ID, Loc::getMessage("SITEMAP_EDIT_TITLE"));
    ...
    $row->AddField("NAME", '<a href="longbyte_sitemap_edit.php?ID=' . $sitemap["ID"] . '&amp;lang=' . LANGUAGE_ID . '" title="' . Loc::getMessage("SITEMAP_EDIT_TITLE") . '">' . Converter::getHtmlConverter()->encode($sitemap['NAME']) . '</a>');
    ...
    $row->AddActions(array(
        array(
            ...
            "ACTION" => $adminList->ActionRedirect("longbyte_sitemap_edit.php?ID=" . $sitemap["ID"] . "&lang=" . LANGUAGE_ID),
            "DEFAULT" => true,
        ),
        array(
            "ICON" => "move",
...
foreach ($arSites as $arRes) {
    $arDDMenu[] = array(
        ...
        "LINK" => "longbyte_sitemap_edit.php?lang=" . LANGUAGE_ID . "&site_id=" . $arRes['LID']
    );
}
...
<script>
    ...
    BX.runSitemap = function (ID, value, pid, NS)
    {
        ...
        BX.ajax.post('/bitrix/admin/longbyte_sitemap_run.php', {
            lang: '<?= LANGUAGE_ID ?>',
            action: 'sitemap_run',
            ID: ID,
            value: value,
    ...
    {
        window.tbl_sitemap.GetAdminList('/bitrix/admin/longbyte_sitemap.php?lang=<?= LANGUAGE_ID ?>');
    };
</script>

...
define('ADMIN_MODULE_NAME', 'longbyte.sitemap');
...
use Bitrix\Longbyte\SitemapRuntime;
...
if (!Main\Loader::includeModule('seo') || !Main\Loader::includeModule(ADMIN_MODULE_NAME)) {
...
function findUrlRule($arRules, $strUrl) {
    ...
    foreach ($arRules as $arRule) {
        $match = false;
        $ruleType = substr($strUrl, 0, 1);
        switch ($ruleType) {
            case '=':
                if ($strUrl == substr($arRule['URL'], 1)) {
                    $match = true;
                }
                break;
            case '~':
                if (preg_match(substr($arRule['URL'], 1), $strUrl)) {
                    $match = true;
                }
                break;
            default :
                if (strpos($strUrl, $arRule['URL']) === 0) {
                    $match = true;
                }
                break;
        }
        if ($match) {
            $arReturn['PRIORITY'] = $arRule['PRIORITY'];
            $arReturn['CHANGEFREQ'] = $arRule['CHANGEFREQ'];
            $arReturn['BANRULE'] = $arRule['BANRULE'] == 'Y';
            echo '<pre>' . $strUrl . ':';
            print_r($arReturn);
            echo '</pre>';
            break;
...
//							remove or replace SERVER_NAME
                            ...
                            $arRule = findUrlRule($arRules, $url);
                            if (!$arRule['BANRULE'])
                                $sitemapFile->addIBlockEntry($url, $elementLastmod, $arRule['PRIORITY'], $arRule['CHANGEFREQ']);
                        }
                    } elseif (!$bCheckFinished) {
                        $bCheckFinished = true;
                        $dbIblockResult = null;
...
//							remove or replace SERVER_NAME
                            ...
                            $arRule = findUrlRule($arRules, $url);
                            if (!$arRule['BANRULE'])
                                $sitemapFile->addIBlockEntry($url, $sectionLastmod, $arRule['PRIORITY'], $arRule['CHANGEFREQ']);
                        }
...
//							remove or replace SERVER_NAME
                            ...
                            $arRule = findUrlRule($arRules, $url);
                            if (!$arRule['BANRULE'])
                                $sitemapFile->addIBlockEntry($url, $NS['IBLOCK_LASTMOD'], $arRule['PRIORITY'], $arRule['CHANGEFREQ']);
                        }
            ...
            if (!$currentForum) {
                ...
                if ($bActive) {
                    ...
                    if (($arTopic = $dbTopicResult->fetch()) && $arTopic) {
                        ...
                        $arRule = findUrlRule($arRules, $url);
                        if (!$arRule['BANRULE'])
                            $sitemapFile->addIBlockEntry($url, MakeTimeStamp($arTopic['LAST_POST_DATE']), $arRule['PRIORITY'], $arRule['CHANGEFREQ']);
                    }
                } else {
                    $url = CForumNew::PreparePath2Message(
                            $arCurrentForum["PATH2FORUM_MESSAGE"], array(
                    ...
                    $arRule = findUrlRule($arRules, $url);
                    if (!$arRule['BANRULE'])
                        $sitemapFile->addIBlockEntry($url, MakeTimeStamp($arCurrentForum['LAST_POST_DATE']), $arRule['PRIORITY'], $arRule['CHANGEFREQ']);
                }
                if (empty($arTopic)) {
                    $bCheckFinished = false;

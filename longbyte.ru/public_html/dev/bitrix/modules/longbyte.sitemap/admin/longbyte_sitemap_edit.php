...
if ($ID > 0) {
    ...
    if (!is_array($arSitemap)) {
        ...
        if ($_REQUEST['action'] == 'delete' && check_bitrix_sessid()) {
            ...
            LocalRedirect(BX_ROOT . "/admin/longbyte_sitemap.php?lang=" . LANGUAGE_ID);
        }
    ...
    if (empty($errors)) {
        ...
        if ($result->isSuccess()) {
            ...
            if ($_REQUEST["save"] <> '') {
                ...
                LocalRedirect(BX_ROOT . "/admin/longbyte_sitemap.php?lang=" . LANGUAGE_ID . "&run=" . $ID . "&" . bitrix_sessid_get());
            } else {
                LocalRedirect(BX_ROOT . "/admin/longbyte_sitemap_edit.php?lang=" . LANGUAGE_ID . "&ID=" . $ID . "&" . $tabControl->ActiveTabParam());
            }
        } else {
...
// load iblock structure
    ...
    $aMenu[] = array(
        ...
        "LINK" => "/bitrix/admin/longbyte_sitemap.php?lang=" . LANGUAGE_ID,
        "ICON" => "btn_list",
    ...
    if ($ID > 0) {
        $aMenu[] = array(
            ...
            "LINK" => "javascript:if(confirm('" . Loc::getMessage("SITEMAP_DELETE_CONFIRM") . "')) window.location='/bitrix/admin/longbyte_sitemap_edit.php?action=delete&ID=" . $ID . "&lang=" . LANGUAGE_ID . "&" . bitrix_sessid_get() . "';",
            "ICON" => "btn_delete",
    ...
    <form method="POST" action="<?= POST_FORM_ACTION_URI ?>" name="sitemap_form">
        ...
        $bChecked = isset($arSitemap['SETTINGS']['DIR']) ? $arSitemap['SETTINGS']['DIR'][$startDir] == 'Y' : true;
        ?>
        <script>
            var loadedDirs = {};
